// This file was copied directly from the macos_ui package, v2.2.2 (MIT license)
// and then modified with custom edits.
...
class _MacosPulldownRoute extends PopupRoute {
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight) {
    final double computedMaxHeight = availableHeight - 2.0 * _kMenuItemHeight;
...
class _MenuItem extends SingleChildRenderObjectWidget {
  // ignore: use_super_parameters
  const _MenuItem({required this.onLayout, Key? key, this.item}) : super(key: key, child: item);
...
/// An entry in a menu created by a [CustomPulldownButton]. It can be either a
...
/// A divider (horizontal line) in a menu created by a [CustomPulldownButton].
...
/// An item in a menu created by a [CustomPulldownButton], typically a [Text]
/// widget.
...
class CustomPulldownButton extends StatefulWidget {
  ...
  @override
  State<CustomPulldownButton> createState() => _CustomPulldownButtonState();
}

class _CustomPulldownButtonState extends State<CustomPulldownButton> with WidgetsBindingObserver {
  ...
  @override
  void didUpdateWidget(CustomPulldownButton oldWidget) {
  ...
  Future<void> _handleTap() async {
    ...
    await navigator.push(_pulldownRoute!).then<void>((_) {
      if (!mounted) {
        return;
      }
...
_ButtonStyles _getButtonStyles(
  ...
  final Color iconColor = pulldownTheme.iconColor!;
  if (!enabled) {
